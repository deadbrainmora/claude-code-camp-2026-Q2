# Player Memory

## Character Stats
- Name: dummy
- Level: 1 (Swordpupil)
- Experience: 246/2000 (need 1754 more to level up)
- Location: Poor Alley
- Health: 20/20
- Mana: 100/100
- Movement: 83/85
- Gold: 30 coins
- Armor Class: 39/10
- Alignment: 44
- Age: 17 years old

## Inventory
- (empty)

## Equipment
- light: a candle
- fingers: leather ring (x2)
- neck: leather gorget (x2)
- body: breast plate
- head: leather cap
- legs: bronze leggings
- feet: leather boots
- hands: leather gloves
- arms: leather sleeves
- shield: shield
- about body: brown leather cape
- about waist: old leather belt
- wrist: leather wristguard (x2)
- wielded: small sword
- held: metal staff

## Long-Term Goals
- [ ] **Reach Level 7** (Priority: 1)
  - Grind in starting area to level 2-3 (need ~2000 exp, currently at 246/2000)
  - Defeat level 2-3 monsters for mid-level experience
  - Learn first spell or combat skill
  - Explore and defeat enemies in new areas (levels 3-5)
  - Reach level 6 prerequisites
  - Final grind to level 7
  > Currently level 1, 246 exp (1754 more to level 2). Well-equipped with starter gear.

- [ ] **Defeat a Specific Monster** (Priority: 2)
  - Discover and locate target monster
  - Learn its difficulty and loot table
  - Gather appropriate equipment and healing items
  - Develop combat strategy
  - Execute fight
  > Need to identify which monster to target first

- [ ] **Explore and Map Starting Region** (Priority: 3)
  - Visit all accessible locations from starting area
  - Document exits and connections
  - Find at least 3 NPCs and talk to them
  - Identify all monster types in the region
  > Just started, no exploration yet

## Skills & Spells
### Skills
- (none learned)

### Spells
- (none learned)

*Last updated: 2024-07-23T22:45:00.000Z*
