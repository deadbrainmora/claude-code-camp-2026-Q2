...
> **DOCUMENTATION ONLY — this file is not loaded at runtime.**
>
> The `play-mud` subagent is defined programmatically in [`agents.py`](../../agents.py)
> as an `AgentDefinition`, and `run.py` passes `setting_sources=[]` so nothing under
> `.claude/` is read. Edit `agents.py` to change the agent's behavior; this file is
> kept as a readable reference and will not take effect on its own.
>
> The former `type:`, `scripts:`, and `data:` frontmatter keys were removed — they
> were never real subagent fields and were silently ignored even when this file was
> being loaded. The script and data paths now live in `agents.py` as
