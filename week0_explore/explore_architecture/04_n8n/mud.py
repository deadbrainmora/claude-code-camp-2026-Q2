"""Programmatic subagent definitions for the MUD player.

This module is the source of truth for the `play-mud` subagent. It replaces
loading the agent off disk from `.claude/agents/play-mud.md`, which is kept
only as human-readable documentation and is not read at runtime (see the
`setting_sources=[]` option in run.py).

Naming: our own constants are all prefixed `PLAY_MUD_*`, with `AGENTS` as the
registry (matching the `agents=` option it is passed to).

The SDK itself is split and we can't change that: `ClaudeAgentOptions` is
entirely snake_case (`allowed_tools`, `setting_sources`), while
`AgentDefinition` keeps camelCase for its multi-word fields (`disallowedTools`,
`maxTurns`, `permissionMode`) to match the wire format. Verified against
claude-agent-sdk 0.2.128. None of those multi-word fields are used here, so no
camelCase actually appears below.
"""

from claude_agent_sdk import AgentDefinition

# Every constant describing the play-mud agent carries the PLAY_MUD_ prefix, so
# adding a second agent later means a parallel set of SECOND_AGENT_* constants
# rather than a scramble over which generic name belongs to which agent.
#
# The agent shells out to these Node scripts and maintains these state files.
# Paths are relative to the project root, which run.py sets as `cwd`.
PLAY_MUD_SCRIPTS_DIR = ".claude/agents/scripts"
...
```bash
cd {PLAY_MUD_SCRIPTS_DIR} && node mud_cli.js <action> [params]
```
...
```bash
cd {PLAY_MUD_SCRIPTS_DIR} && node mud_cli.js send "look"
```

**Run a sequence of commands:**
```bash
cd {PLAY_MUD_SCRIPTS_DIR} && node mud_cli.js sequence "north" "look" "kill goblin"
```

**Interactive session:**
```bash
cd {PLAY_MUD_SCRIPTS_DIR} && node mud_cli.js interactive
```
...
- **connect**: Establish telnet connection and login
...
2. Verify the player account is created and accessible
3. Node.js 14+ must be installed
4. The connection, parser, and CLI scripts are in `{PLAY_MUD_SCRIPTS_DIR}/`

### Agent Initialization
When invoking the agent:
- Scripts reference each other with relative requires (they must run from the scripts directory)
- Data files are updated in `{PLAY_MUD_DATA_DIR}/` to maintain state between sessions
...
When executing mud_cli.js, ensure the working directory is `{PLAY_MUD_SCRIPTS_DIR}/`:

```bash
cd {PLAY_MUD_SCRIPTS_DIR}
node mud_cli.js <action> [params]
```

Alternatively, use absolute paths if calling from a different directory:
```bash
node /absolute/path/to/{PLAY_MUD_SCRIPTS_DIR}/mud_cli.js send "look"
```

### State Management
- Player memory is stored in `{PLAY_MUD_DATA_DIR}/player.md`
- World knowledge is stored in `{PLAY_MUD_DATA_DIR}/world.md`
...
- Connection state is local to this session — if the runner crashes, you'll need to reconnect
...
# The agent only needs to shell out to the Node CLI and maintain its two state
# files, so it gets a narrowed tool set rather than inheriting everything a
# subagent can normally reach (no WebFetch/WebSearch, no nested Agent).
...
# Registry passed straight to ClaudeAgentOptions(agents=...). Named to match
# that option; the dict keys are the names Claude invokes agents by.
AGENTS = {
